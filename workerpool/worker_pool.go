...
func (wp *workerPool) run(ctx context.Context) {
	...
	var todoTask *Task
	defer wp.wg.Done()
	// exit in two situation
	// 1. cancel context from high parent
	// 2. when all task are done, task channel will be close, so will read NIL from channel
	for {
		select {
		case <-ctx.Done():
			return
		default:
			todoTask = <-wp.tasks
			if todoTask != nil {
				wp.results <- todoTask.Func(todoTask.Args...)
			} else {
				return
			}
		}
